...
class GovernorFirestoreService {
  ...
  Future<GovernorFirestoreSnapshot> loadProvincialSnapshot() async {
    ...
    if (!isStaff) {
      ...
      debugPrint(
        '[GovernorFirestore] skip provincial load — not staff (uid=$uid)',
      );
      return GovernorFirestoreSnapshot(
        ...
        announcements: const [],
        loadWarnings: const [
          'Governor dashboard requires a governor or tourism staff account.',
        ],
      );
    }
  ...
  Future<List<Map<String, dynamic>>> _loadAllCheckIns(
    ...
    Future<void> ingest(
      ...
      try {
        ...
        if (!legacyOptional) {
          warnings.add('Could not load all $collection records.');
        }
      }
    }
  ...
  void _mergeCheckInDocs(
    ...
    for (final doc in docs) {
      ...
      // Prefer document id so the same qr_checkins row is never counted twice
      // when municipality chunk fallback also runs.
      byId[doc.id] = normalized;
    }
  }
