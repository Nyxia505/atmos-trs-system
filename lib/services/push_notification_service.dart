...
Future<void> _ensureLocalNotificationsCore() async {
  ...
  if (Platform.isIOS) {
    // System banner OR local plugin — not both (avoids duplicate heads-up).
...
Future<void> syncFcmTokenToUserDoc(String uid) async {
  ...
  try {
    ...
    // Subscribe early so topic pushes arrive without waiting for MainShell.
    try {
      await FirebaseMessaging.instance.subscribeToTopic(kGovernorAnnouncementsTopic);
    } catch (e) {
      debugPrint('[Push] early topic subscribe: $e');
    }
    ...
    // Also store on tourists/{uid} for multicast coverage.
    try {
      await FirebaseFirestore.instance.collection('tourists').doc(uid).set(
        patch,
        SetOptions(merge: true),
...
void _startAnnouncementFallbackListener() {
  ...
  unawaited(() async {
    ...
    _announcementFallbackSubscription = query.snapshots().listen((snapshot) async {
      ...
      // First snapshot only seeds last-seen — do not notify for existing posts.
      if (!primed) {
        primed = true;
      ...
      final shouldNotify = _lastSeenPublishedAnnouncementId != id;
      _lastSeenPublishedAnnouncementId = id;
      await _persistLastSeenAnnouncementId(id);
      ...
      final safeTitle = (title == null || title.isEmpty) ? 'ATMOS-TRS' : title;
      final safeBody = content.isEmpty ? 'New announcement' : content;
      ...
      // Local heads-up when FCM is delayed/unavailable; skipped if FCM already showed.
      if (_localNotificationsReady) {
        await _showAnnouncementHeadsUpLocal(
          announcementId: id,
          title: safeTitle,
