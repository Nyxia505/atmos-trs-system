...
class TouristActivityFirestoreSync {
  ...
  /// Call when the signed-in tourist changes so cloud merge runs again.
  static void resetMergeCache() {
    _lastMergedUid = null;
  }
  ...
  static Future<void> mergeFromCloud(String uid) async {
    ...
    try {
      ...
      // Visited places come from qr_checkins — not tourist_activity backup rows.
      final mergedV = await UserActivityService.syncVisitedSpotsFromQrCheckins();
