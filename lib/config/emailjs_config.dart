...
///
/// **404 "Account not found":** almost always means `publicKey` does not match
/// your EmailJS account (typo, extra space, or keys were **refreshed** in
/// Account → API keys). Copy **Public Key** again — never use **Private Key**
/// as `user_id` (private is optional `accessToken` for server-side only).
class EmailjsConfig {
  EmailjsConfig._();

  /// Gmail service ID from EmailJS → Email Services.
  static const String serviceId = 'service_fuwpuo4';

  /// Template ID from EmailJS → Email Templates (OTP: subject e.g. "ATMOS-TRS OTP code").
  /// Body placeholders: `{{to_name}}`, `{{otp}}` — also send `to_email` for the To field.
  static const String templateId = 'template_fk8jzbr';

  /// Public Key from EmailJS → Account → API keys (client-side send only).
  static const String publicKey = '9nyn4calqpjJxcgE4';
  ...
  static const String privateAccessToken = '';
}
